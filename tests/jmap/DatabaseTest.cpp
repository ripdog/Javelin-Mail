...
TEST_CASE("database connection creates the initial cache schema", "[jmap][cache][database]")
{
    ...
    CHECK(migrations.back().version == 21);
    CHECK(migrations.back().name == QStringLiteral("generic_mutation_journal"));
          ...
          "[jmap][cache][database]")
{
    ...
    CHECK(migrations.at(6).version == 9);
    CHECK(migrations.back().version == 21);
    CHECK(connection.schemaVersion() == 21);
}
