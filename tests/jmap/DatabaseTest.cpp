...
TEST_CASE("database connection creates the initial cache schema", "[jmap][cache][database]")
{
    ...
    CHECK(migrations.back().version == 16);
    CHECK(migrations.back().name == QStringLiteral("observed_notification_emails"));
    ...
    CHECK(tableNames ==
          QStringList{QStringLiteral("accounts"), QStringLiteral("compose_sessions"),
                      ...
                      QStringLiteral("mailboxes"), QStringLiteral("observed_notification_emails"),
          ...
          "[jmap][cache][database]")
{
    ...
    CHECK(migrations.at(6).version == 9);
    CHECK(migrations.back().version == 16);
    CHECK(connection.schemaVersion() == 16);
}
