...
namespace javelin::gui::calendar
{
    ...
    javelin::jmap::calendar::CalendarEvent EventDialog::eventDocument() const
    {
        ...
        if (m_notifications->edited())
        {
            result.useDefaultAlerts = m_notifications->useCalendarDefaults();
            for (auto alert = result.alerts.begin(); alert != result.alerts.end();)
            {
                if (alert->second.action == "display" &&
                    alert->second.triggerKind == javelin::jmap::calendar::AlertTriggerKind::Offset)
                    alert = result.alerts.erase(alert);
                else
                    ++alert;
            }
            for (auto [id, alert] : m_notifications->displayAlerts())
                result.alerts.insert_or_assign(std::move(id), std::move(alert));
        }
        return result;
    }
