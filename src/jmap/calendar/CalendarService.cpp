...
namespace javelin::jmap::calendar
{
    ...
    namespace
    {
        ...
        simpleRecurrenceCandidate(const QDateTime& start, const RecurrenceRule& rule,
                                  const std::uint64_t periodIndex)
        {
            constexpr std::uint64_t maximumPeriodIndex = 100000;
            if (periodIndex > maximumPeriodIndex || rule.interval == 0)
                return std::nullopt;
            const auto offset =
                static_cast<qint64>(periodIndex) * static_cast<qint64>(rule.interval);
            switch (rule.frequency)
            {
            case RecurrenceFrequency::Daily:
                return start.addDays(offset);
            case RecurrenceFrequency::Weekly:
                return start.addDays(offset * 7);
            case RecurrenceFrequency::Monthly:
            {
                const auto absoluteMonth = static_cast<qint64>(start.date().year()) * 12 +
                                           start.date().month() - 1 + offset;
                const auto year = absoluteMonth / 12;
                const auto month = absoluteMonth % 12 + 1;
                ...
                const QDate date{static_cast<int>(year), static_cast<int>(month),
                                 start.date().day()};
                return date.isValid() ? std::optional{QDateTime{date, start.time()}} : std::nullopt;
            }
            case RecurrenceFrequency::Yearly:
            {
                const auto year = static_cast<qint64>(start.date().year()) + offset;
                if (year < std::numeric_limits<int>::min() ||
                    year > std::numeric_limits<int>::max())
                    return std::nullopt;
                const QDate date{static_cast<int>(year), start.date().month(), start.date().day()};
                return date.isValid() ? std::optional{QDateTime{date, start.time()}} : std::nullopt;
            }
        ...
        [[nodiscard]] std::optional<std::vector<Occurrence>>
        expandSimpleRecurrence(const CalendarEvent& event, const VisibleInterval& interval)
        {
            if (!event.recurrenceRule || event.recurrenceRule->hasUnsupportedExpansionProperties ||
                event.recurrenceRule->frequency == RecurrenceFrequency::Hourly ||
                event.recurrenceRule->frequency == RecurrenceFrequency::Minutely ||
                event.recurrenceRule->frequency == RecurrenceFrequency::Secondly)
                return std::nullopt;
            const auto seriesStart =
                QDateTime::fromString(QString::fromStdString(event.start.value), Qt::ISODate);
            const auto rangeStart =
                QDateTime::fromString(QString::fromStdString(interval.start.value), Qt::ISODate);
            const auto rangeEnd =
                QDateTime::fromString(QString::fromStdString(interval.end.value), Qt::ISODate);
            const auto until =
                event.recurrenceRule->until
                    ? QDateTime::fromString(
                          QString::fromStdString(event.recurrenceRule->until->value), Qt::ISODate)
                    : QDateTime{};
            if (!seriesStart.isValid() || !rangeStart.isValid() || !rangeEnd.isValid() ||
                (event.recurrenceRule->until && !until.isValid()))
                return std::nullopt;

            std::vector<Occurrence> result;
            std::unordered_set<std::string> processedRecurrenceIds;
            const auto append =
                [&](const std::string& recurrenceId, const RecurrenceOverride* recurrenceOverride)
            {
                if (recurrenceOverride && recurrenceOverride->excluded)
                    return;
                auto occurrenceEvent = event;
                occurrenceEvent.start = recurrenceOverride && recurrenceOverride->start
                                            ? *recurrenceOverride->start
                                            : LocalDateTime{.value = recurrenceId};
                if (recurrenceOverride && recurrenceOverride->duration)
                    occurrenceEvent.duration = *recurrenceOverride->duration;
                const auto actualStart = QDateTime::fromString(
                    QString::fromStdString(occurrenceEvent.start.value), Qt::ISODate);
                const auto actualEnd = QDateTime::fromString(
                    QString::fromStdString(localEnd(occurrenceEvent).value), Qt::ISODate);
                if (!actualStart.isValid() || !actualEnd.isValid() || actualEnd <= rangeStart ||
                    actualStart >= rangeEnd)
                    return;
                result.push_back({.accountId = event.accountId,
                                  .id = event.id + "/" + recurrenceId,
                                  .eventId = event.id,
                                  .recurrenceId = LocalDateTime{.value = recurrenceId},
                                  .localStart = occurrenceEvent.start,
                                  .localEnd = localEnd(occurrenceEvent),
                                  .utcStart = std::nullopt,
                                  .utcEnd = std::nullopt,
                                  .allDay = event.showWithoutTime});
            };

            constexpr std::uint64_t maximumPeriods = 100000;
            std::uint64_t generatedCount = 0;
            for (std::uint64_t periodIndex = 0; periodIndex < maximumPeriods; ++periodIndex)
            {
                const auto candidate =
                    simpleRecurrenceCandidate(seriesStart, *event.recurrenceRule, periodIndex);
                if (!candidate)
                    continue;
                if (until.isValid() && *candidate > until)
                    break;
                if (event.recurrenceRule->count && generatedCount >= *event.recurrenceRule->count)
                    break;
                ++generatedCount;
                const auto recurrenceId = candidate->toString(Qt::ISODate).toStdString();
                processedRecurrenceIds.insert(recurrenceId);
                const auto recurrenceOverride = event.recurrenceOverrides.find(recurrenceId);
                append(recurrenceId, recurrenceOverride == event.recurrenceOverrides.end()
                                         ? nullptr
                                         : &recurrenceOverride->second);
                if (*candidate >= rangeEnd)
                    break;
            }
            for (const auto& [recurrenceId, recurrenceOverride] : event.recurrenceOverrides)
                if (!processedRecurrenceIds.contains(recurrenceId))
                    append(recurrenceId, &recurrenceOverride);
            return result;
        }
        ...
        for (const auto& [accountId, account] : session.accounts)
        {
            ...
            for (const auto& occurrence : occurrences)
                if (occurrence.recurrenceId)
                    occurrenceKeys.insert(occurrence.eventId + "\n" +
                                          occurrence.recurrenceId->value);
            for (const auto& event : baseResponse.list)
            {
                if (!event.recurrenceRule || rebased.suppressedExpansionIds.contains(event.id))
                    continue;
                const auto localOccurrences = expandSimpleRecurrence(event, interval);
                if (!localOccurrences)
                    continue;
                std::size_t supplemented = 0;
                for (const auto& occurrence : *localOccurrences)
                {
                    if (!occurrence.recurrenceId)
                        continue;
                    const auto key = occurrence.eventId + "\n" + occurrence.recurrenceId->value;
                ...
                if (supplemented > 0)
                    qCWarning(logCalendarService).noquote()
                        << "supplement server-omitted calendar occurrences"
                        << QString::fromStdString(accountId) << QString::fromStdString(event.id)
                        << QString::fromStdString(event.uid) << supplemented;
            }
