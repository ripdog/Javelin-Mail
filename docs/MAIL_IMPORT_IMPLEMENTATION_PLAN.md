...
This document defines the implementation plan for importing mail into Javelin from local mail files:
raw `.eml` messages and conventional mbox-family mailbox files, singly or arranged in directory
trees. See [MAIL_EXPORT_IMPLEMENTATION_PLAN.md](MAIL_EXPORT_IMPLEMENTATION_PLAN.md) for the matching
export feature.

Import/export is an interoperability feature, not a full JMAP account backup/restore mechanism. Its
primary purpose is moving mail between Javelin and other local email tools. It therefore preserves
message content, useful folder placement, and chronology where practical, but does not attempt to
reconstruct JMAP object identity, mutable metadata, or the exact membership graph of the source
account.
...
- The cross-server transfer executor already implements much of the per-item pipeline shape import
  needs: source acquisition -> blob upload -> destination state capture -> `Email/import` ->
  per-item durable phase transitions -> retryable-error classification into waiting states ->
  ambiguous-dispatch handling. Import reuses those low-level patterns without inheriting transfer's
  compare-and-swap policy where it is unnecessary.
...
- Import single `.eml` files, directories of `.eml` files, conventional mbox-family mailbox files,
  and directory trees containing either layout, including trees produced by Javelin account/mailbox
  export.
- Preserve RFC 5322 message content end to end. `.eml` bytes are uploaded unchanged; mbox records are
  decoded only to remove container framing/quoting before upload. Do not parse and reconstruct MIME,
  refold headers, or otherwise transform message content.
...
- Treat duplicate handling as destination-server policy. Each source record represents one import
  attempt; if the server reports `alreadyExists`, ensure the existing Email has the requested
  destination mailbox membership and report that the message was reused rather than newly created.

## Non-goals

- Do not support Maildir, PST, iCalendar, compressed archives, or proprietary mailbox databases in
  the initial implementation. For mbox, support the conventional separator-based variants that can
  be parsed safely with bounded scanning; do not promise compatibility with every historical mbox
  dialect.
- Do not restore keywords/read/starred state. Portable `.eml`/mbox files do not provide a reliable,
  cross-client representation of JMAP mutable state. Private client headers such as
  `X-Mozilla-Status` are ignored rather than interpreted as authoritative JMAP metadata.
...
1. A directory is classified as an EML tree if it recursively contains message files treated as
   RFC 5322 messages, or as an mbox tree if it contains mbox mailbox files. Javelin's paired mbox
   export layout (`Projects.mbox` plus sibling `Projects/` containing child mailbox files) is
   recognized explicitly. Ambiguous mixed EML/mbox trees are rejected rather than risking duplicate
   imports from two competing representations.
2. A regular file whose first bytes form a plausible `From_` envelope separator is parsed as one
   mbox-family mailbox regardless of extension (this accepts `.mbx` and extensionless mbox files).
...
The reader uses conventional separator-based mbox framing and is an exact inverse of Javelin's
`appendMboxRdRecord()` for the escaping rule it emits:

- Records are delimited by plausible `From_` separator lines at the start of a line, scanned
  incrementally over bounded buffers. The parser accepts the normal mbox form and must not depend on
  Javelin-specific filenames or `Content-Length:` framing hints.
- Every body/message line consisting of one or more `>` characters followed by `From ` loses exactly
  one leading `>`. This reverses mboxrd quoting and exactly restores lines written by Javelin's
  exporter.
...
- **File -> Import Folder Tree...** opens a directory chooser plus destination account and target
...
- Missing ancestor mailboxes are created under the target parent through the existing durable mailbox
  mutation machinery. The import journal owns the workflow dependency and records the resolved
  mailbox id, while `MailboxMutationEngine` owns `Mailbox/set`, ambiguity reconciliation, and cache
  consistency. These workflow-driven creations produce no Undo entries.
- Directory components are treated as the mailbox names represented by the local filesystem layout;
  sanitization performed during export is not invertible and import does not pretend otherwise.
  Reject or normalize only names the destination server cannot represent safely, with any resulting
  rename made visible in the summary.
- Before creating a mailbox for a path segment, resolve against existing sibling mailboxes by exact
  user-visible name. Repeated imports may reuse an exact existing folder; names that merely sanitize
  or case-fold similarly are not assumed to represent the same mailbox unless the destination
  server itself treats them as equivalent.
...
Duplicate semantics deliberately follow the destination server rather than trying to infer source
identity from message bytes:

- Every source record is one import attempt. Do not suppress later records merely because their
  SHA-256 matches an earlier record; two distinct source messages may legitimately have identical
  RFC 5322 content.
- If `Email/import` succeeds, record the newly created Email id.
- If the server reports `alreadyExists` with `existingId`, fetch that Email authoritatively. If it is
  already in the requested destination mailbox, the placement is satisfied. Otherwise add only the
  requested destination mailbox membership with an exact `Email/set` patch, preserving the existing
  Email's keywords and other mutable metadata.
- Result summaries distinguish newly created messages from existing messages reused to satisfy the
  requested placement.

Portable EML/mbox import does not promise exact reconstruction of a source account's JMAP identity or
multi-mailbox membership graph. If the same message appears in multiple source folders, the result may
be one Email with multiple memberships or multiple Emails depending on destination-server duplicate
policy. That is acceptable interoperability behaviour.
...
struct MailImportIntent {
    ...
    std::vector<QString> sourcePaths;       // ordered files/directories in one user operation
    bool recreateHierarchy = false;        // DirectoryTree only
};
```

Multiple dropped/chosen paths are admitted as one import operation with an ordered list of source
roots. The journal records the source root for each discovered item so progress, pause/resume, and
partial results remain one coherent user action.

The daemon validates at admission:

- connection-qualified account exists and resolves to live session settings;
- source paths exist, are readable regular files/directories, contain no symlink escapes outside the
  chosen roots during enumeration, and pass content detection; ownership is not required because
  shared/group/NFS-readable mail files are valid import sources;
...
item fields
    ...
    source_sha256              content fingerprint for diagnostics/reconciliation, not source identity
    ...
    phase                      Pending | Reading | Uploading | Uploaded |
                               Creating | Created | ReusedExisting | NoDestination | Failed
...
1. **Reading** — open the source message. For `.eml`, the source is the whole file. For mbox, expose
   one decoded RFC 5322 record through a bounded streaming source that removes the `From_` separator
   and reverses mbox quoting without materializing the whole message. Compute SHA-256 while
   streaming for diagnostics and ambiguity correlation.
2. **Uploading** — stream that RFC 5322 source as `message/rfc822`. Extend the blob-upload layer with
   a bounded streaming-source abstraction rather than assuming every upload is an entire local file;
   `uploadBlobFromFile()` remains the file-backed convenience path. Enforce negotiated
   `maxSizeUpload` against the decoded message size. An oversized message fails definitively for that
   item and the job continues.
3. **Preflight** — capture the destination `Email` state used only as an ambiguity-reconciliation
   anchor; ordinary additive imports do not require compare-and-swap semantics.
4. **Creating** — submit one `Email/import` creation per durable scheduler admission without
   `ifInState`. The conservative single-item admission keeps ambiguity accounting exact while still
   allowing the scheduler to interleave other work. `alreadyExists` triggers
   authoritative existing-Email reconciliation and, if needed, an exact mailbox-membership add;
   `overQuota` pauses the job with a quota-specific detail; other SetErrors mark the item failed.
5. **Recording** — transition the item phase and persist the created/reused Email id atomically
   before advancing past it.

Creation handling:

- Imports are additive and do not use `ifInState`, so unrelated successful imports do not manufacture
  `stateMismatch` failures against one another. Capture pre-dispatch Email state solely for ambiguity
  reconciliation.
...
- The cross-server transfer executor establishes the accepted pattern for bulk durable workflows:
  typed journal + pre-dispatch state capture where needed for reconciliation + authoritative
  synchronization. Import does not use `ifInState` merely as a generic concurrency guard.

The service requests targeted authoritative synchronization periodically during long imports and at
terminal/waiting boundaries so imported messages materialize into cache promptly without issuing a
refresh per message. All cache writes
...
The normal import path reads source files in place and streams decoded mbox records directly to the
upload transport, so it does not require per-message staging files. Local failures can still occur if
the source filesystem disappears or changes underneath the job. `overQuota` responses pause the job
with a message naming the account; resume retries after the user frees server space.
...
1. **Re-import duplication after crash or ambiguity.** The worst import bug is silently creating a
   second copy because Javelin lost the first response. *Mitigation:* per-item durable phases are
   committed before advancing; ambiguous outcomes become `unknown` and reconcile against the server
   before resubmission; server `alreadyExists` is resolved through `existingId` rather than treated
   as failure; acceptance tests restart the job at every item phase including mid-batch and
   mid-upload. Content equality alone is not used as source-record identity.

2. **mbox unescaping corruption.** Stripping the wrong number of `>` characters corrupts bodies or
   splits mailboxes. *Mitigation:* strip exactly one `>` from quoted lines matching `^>+From `;
   round-trip property tests verify Javelin-exported records restore their original lines. Tests
   separately account for unavoidable mbox record framing changes such as an exporter-added final
   newline.
...
4. **Foreign mbox variants.** Other clients produce mboxo/mboxrd/mboxcl-family files with slightly
   different quoting conventions, and `Content-Length:` headers are frequently stale after edits.
   *Mitigation:* frame records by validated `From_` separator scanning, ignore `Content-Length` as an
   authoritative boundary, apply mboxrd unquoting where present, and maintain a fixture corpus from
   common desktop mail tools. Reject ambiguous/corrupt framing instead of guessing across data.
...
9. **Hierarchy collisions on recreate.** Filesystem-safe exported names may not reveal the exact
   original mailbox name, and destination servers may reject or case-fold names differently.
   *Mitigation:* treat on-disk components as the imported mailbox names, reuse exact existing sibling
   names only, surface any destination-required rename, and contain creation failure to the affected
   subtree. Import does not claim to invert export sanitization.
...
11. **Vanishing or changing sources.** Files can disappear, be replaced, or change after admission;
    persisted mbox record offsets are especially dangerous if the container is rewritten while the
    job is stopped. *Mitigation:* the sealed scan records a durable source fingerprint (filesystem
    identity where available, size, high-resolution modification time, and content fingerprint as
    needed). Every later read revalidates the source before trusting saved offsets. A changed mbox
    source is invalidated/rescanned or failed explicitly rather than reading stale offsets;
    disappearance fails only the affected source/items.
...
- export `.eml` -> import -> uploaded RFC 5322 hash equality for fixture corpora;
- export mboxrd tree -> import with hierarchy recreation -> equivalent decoded message content and
  isomorphic on-disk folder topology, subject to documented mbox framing transformations and
  destination-server duplicate policy;
- mboxrd export -> mboxrd import round-trip preserves message content modulo the documented record
  framing/newline transformation.

### Pipeline and durability

- each scheduler admission attempts at most one Email creation and records its outcome durably;
- additive `Email/import` does not rely on `stateMismatch` retry semantics;
...
- identical-content source records are each attempted; destination `alreadyExists` reuses the
  existing Email and ensures requested mailbox placement;
...
- filesystem/path-hostile components remain contained and any destination-required rename is
  deterministic and reported;
- exact existing-folder names are reused; merely sanitized/case-fold-equivalent names are not
  silently merged unless required by destination semantics;
- `mayCreateChild` denial fails only the affected subtree;
- interrupted mailbox creation resumes without duplicates.

## Implementation order

1. Add the mbox-family reader/source enumerator and a bounded decoded-message stream abstraction,
   with fixtures from the existing exporter and representative external desktop-mail mbox files.
2. Add the import journal repository and schema migration, plus `WorkKind::MailImport`
   reconstruction, with no GUI caller.
3. Extend blob upload to accept bounded streaming message sources, then implement
   `MailImportApplicationService`: scan/admission, bounded pipeline, waiting states, durable
   single-item creation admissions, existing-Email placement, ambiguity reconciliation, and
   coalesced synchronization demand.
...
1. Imported RFC 5322 content reaches the server unchanged after removal of container-only mbox
   framing; `.eml` uploads are byte-for-byte identical to the source file. Any further transformation
   happens server-side and is recorded, not hidden.
2. No bulk import path loads a whole message or mailbox file into memory.
3. Restart, ambiguity, and retry never duplicate a message because of an uncertain prior dispatch.
4. Destination-server duplicate outcomes are visible; an existing Email is given the requested
   mailbox placement without overwriting its unrelated mutable state, and a partial import is never
   labeled complete.
5. Received times survive import for mbox-derived messages and follow server Received-header
   semantics for `.eml`.
6. Hierarchy recreation preserves the useful on-disk folder topology, is collision-safe and
   rights-checked, and contains failures per subtree without pretending filesystem sanitization is
   reversible.
