...
- [ ] Round-trip an optimistic-projection-only daemon change through the real IPC codec.
- [ ] Round-trip a new-mail-only daemon change if that semantic still exists at this point.
- [ ] Round-trip a change carrying both flags.
- [ ] Round-trip a change carrying neither flag.
- [ ] Assert that the GUI never invents one semantic merely because another maps to
...
- [ ] Audit all remaining consumers of `MailCacheChange::hasNewMail`.
- [ ] If notification discovery is its only real semantic consumer, remove it from generic cache
      invalidation.
- [ ] If some UI still needs an explicit new-mail event, model that as a dedicated typed event rather
      than inferring it from generic metadata invalidation.
- [ ] Ensure no GUI code reconstructs "new mail" from `ChangedDomain::MessageMetadata`.

## 5.2 Preserve `optimisticProjection` explicitly only if useful

- [ ] Audit whether GUI/session consumers genuinely need to know that a committed cache state contains
      an unresolved optimistic projection.
- [ ] If they do, add/retain an explicit IPC field and round-trip it faithfully.
- [ ] If they do not, remove the dead semantic rather than deriving it from `hasNewMail` or metadata
      domains.
- [ ] Never set `optimisticProjection = hasNewMail` as a reconstruction shortcut.

## 5.3 Stop manufacturing fake changed domains

- [ ] Remove the fallback that emits `MailQueryWindows` when the actual changed-domain set is empty.
- [ ] Add a precise `MailTags` or equivalent domain if tag-definition changes require GUI cache
      invalidation.
- [ ] Do not emit a fake mail-query invalidation for cache changes with no query-window effect.
- [ ] Add focused codec/consumer tests for every new or changed domain.

## 5.4 Keep the wire shape small

- [ ] Retain the existing useful typed fields such as account ID, mailbox IDs, mailbox/search windows,
      message-content Email IDs, changed domains, and epoch.
- [ ] Add/remove only fields required to preserve real semantics.
- [ ] Do not turn this phase into a wholesale cache-event protocol redesign.

## 5.5 Validate Phase 5

- [ ] Run IPC round-trip tests.
- [ ] Run GUI invalidation/session tests.
- [ ] Confirm notification behavior is unchanged because it no longer depends on generic cache
      invalidation.
- [ ] Run `scripts/check-debug.sh --full`.
...
- [ ] Record the current ordering guarantees at the daemon IPC boundary.
- [ ] Determine whether the invalidation can be synchronously allocated/enqueued before command
      completion without changing user-visible latency.
- [ ] Determine whether socket delivery can allow the command reply to be observed before its
      invalidation.
- [ ] Do not introduce a new barrier framework before this test demonstrates a need.
...
- [ ] Reuse the daemon's existing cache/boundary epoch if it can represent this ordering.
- [ ] Ensure the epoch associated with command completion is post-commit, not the previous
      `m_currentEpoch()` observed before the pending zero-timer invalidation is allocated.
- [ ] Keep optimistic state visible immediately from the committed local cache event; do not wait for
...
- [ ] Temporarily hold that command completion until the GUI has observed invalidation epoch `>= E`.
- [ ] Apply this only to projection-producing actions that need presentation ordering.
- [ ] Reuse existing timeout/daemon-recovery behavior.
- [ ] Do not introduce a general-purpose distributed consistency/barrier subsystem.

## 6.4 Remove GUI-local projection refreshes

Once ordering is proven:

- [ ] Remove controller-driven `refreshMessageListPreservingSelection()` calls whose only purpose is
      to mirror the daemon's committed optimistic projection.
- [ ] Remove direct `MessageListModel::setEmailRead(...)` mutation used as a second projection path.
- [ ] Remove duplicate expanded-thread refreshes caused solely by command completion when normal
      cache invalidation already supplies the committed state.
- [ ] Remove/reduce `mailboxMembershipChanged`, `messageMetadataChanged`, `junkStateChanged`, and
      `emailMarkedRead` presentation effects where they duplicate cache truth.
- [ ] Retain command-completion signals only for status/error/action lifecycle effects that are not
      cache projection.
- [ ] Ensure search-tab staleness derives from authoritative cache invalidation rather than a parallel
...
- [ ] Enumerate the cache domains/window identities that can change a `MailboxSession` result.
- [ ] Enumerate the cache domains/window identities that can change a `SearchSession` result.
- [ ] Include relevant mailbox/query window replacement.
- [ ] Include relevant Email metadata/membership represented in loaded rows.
- [ ] Include Thread materialization changes where they affect represented/expanded rows.
- [ ] Include tag-definition changes if row rendering depends on them.
- [ ] Exclude unrelated Contacts, Calendar, Identities, and unrelated message-content downloads.

## 7.2 Advance refresh generation only for relevant invalidations

- [ ] Move `refreshGeneration.begin(...)` behind dependency/intersection checks.
- [ ] Keep cache epoch observation separate from cancellation of an in-flight list read if necessary.
- [ ] Ensure a same-account irrelevant invalidation cannot cause needless read cancellation/retry.
- [ ] Keep relevant optimistic metadata changes capable of invalidating the session normally.

## 7.3 Validate Phase 7

- [ ] Contact invalidation does not restart an Inbox list read.
- [ ] Calendar invalidation does not restart an Inbox list read.
- [ ] Unrelated message-content invalidation does not restart an Inbox list read.
- [ ] Relevant mailbox query-window replacement does restart/reload as required.
- [ ] Relevant Thread completion does refresh represented expansion state.
- [ ] Relevant optimistic metadata update does refresh the row.
- [ ] Run `scripts/check-debug.sh --full`.
...
- [ ] Add `expandedThreadIds` or equivalent to mailbox/search tab presentation state.
- [ ] Keep the model capable of applying expansion/collapse, but do not make it the sole durable owner
...
- [ ] On tab rebind, intersect retained expanded Thread IDs with Threads represented by the current
      query/session.
- [ ] Restore valid expansions.
- [ ] Discard expansion IDs no longer represented.
- [ ] Ensure changing query identity/filter semantics does not restore expansion into an unrelated
      result set.
- [ ] Ensure closing the tab naturally drops its expansion state.
...
- [ ] Cover Thread disappearing while the tab is inactive.
- [ ] Run `scripts/check-debug.sh --full`.
...
- [ ] Re-audit daemon consumers of `MessageListSessionFactoryService`.
- [ ] If there is still no daemon consumer, remove its daemon construction/exposure.
- [ ] Keep GUI message-list sessions in `GuiServices`, backed by daemon materialization and read-only
      SQLite access as currently intended.
- [ ] Update architecture documentation/comments so process ownership is obvious.

## 9.2 Rationalize duplicate epoch concepts only if it is genuinely simpler

- [ ] Re-audit the `CacheInvalidationPublisher` internal epoch and daemon boundary epoch after Phase 6.
- [ ] If one has no independent semantic consumer, collapse to one authoritative commit/invalidation
      epoch.
- [ ] If both still have legitimate responsibilities, document them and leave them alone.
- [ ] Do not expand scope for aesthetic purity.

## 9.3 Validate Phase 9

- [ ] Build all daemon and GUI targets.
- [ ] Run architecture/unit tests affected by service construction.
- [ ] Run `scripts/check-debug.sh --full`.
...
- [ ] Confirm whether an Email subject can change while the raw MIME/content hash used by the index
      remains unchanged.
...
- [ ] Keep this as a separate logical commit/project.

## 10.2 Targeted message-content recovery follow-up

- [ ] Audit the path where raw/message-content fetch returns 404 or changes while downloading and
...
- [ ] Keep this separate from the state-ownership and notification changes above.
...
- [ ] 16. Remove `hasNewMail` from generic invalidation or replace its remaining use with a precise
      typed event.
